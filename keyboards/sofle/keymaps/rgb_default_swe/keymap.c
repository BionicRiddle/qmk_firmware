...
//#define OLED_DEBUG // Debug "strcpy(debug_str, "SLAVE RGB ON");"

// debug stirngs for OLED
#ifdef OLED_DEBUG
static char debug_str[OLED_DISPLAY_WIDTH] = "";
#endif
...
#ifdef OLED_DEBUG
static void render_debug(bool master) {
    // print if master
    if (master) {
...
#ifdef OLED_DEBUG
    render_debug(is_keyboard_master());
    return false;
#endif
...
void keyboard_post_init_user(void) {
    ...
    //debug_enable=true;
    local_rgb_brightness = rgb_matrix_get_val();
    rgb_matrix_sethsv(RGB_MATRIX_DEFAULT_HUE, RGB_MATRIX_DEFAULT_SAT, local_rgb_brightness);
